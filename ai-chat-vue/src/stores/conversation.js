...
export const useConversationStore = defineStore('conversation', () => {
        ...
        try {
            ...
            if (raw) {
                ...
                if (Array.isArray(data)) {
                    ...
                    if (data.currentId && sessions.value.some((s) => s.id === data.currentId)) {
    ...
    async function loadMoreMessages(id) {
        ...
        if (!cache || !cache.hasMore || !cache.cursor)
            return;
    ...
    function appendToLastMessage(content, field = 'content') {
        const id = ensureCurrentSessionId();
    ...
    /** 追加文本到指定会话的指定消息 */
    function appendToMessage(sessionId, messageId, content, field = 'content') {
        const cache = ensureCacheForSession(sessionId);
    ...
    function ensureCacheForSession(sessionId) {
